...
│   ├── entity/           39 个实体类（全部继承 com.dwl.common.base.BaseEntity）
│   ├── dto/              11 个请求传输对象（含 dto/ai/ 子包 4 个 AI DTO）
│   └── vo/               9 个视图响应对象（含 HealthVO）
│
├── delta-test-dao        ← 数据访问层
│   └── mapper/           39 个 MyBatis-Plus Mapper 接口
│
...
│   ├── controller/       8 个 REST Controller（含 HealthController）
│   └── websocket/        WebSocketConfig, TaskProgressWebSocketHandler
│
├── delta-test-grpc-client ← gRPC 客户端模块（Java→C）
│   ├── proto/            code_analysis.proto (3 个 RPC 方法)
│   ├── client/           EngineGrpcClient（computeDiff/analyzeImpact/healthCheck）
│   └── config/           GrpcClientConfig (ManagedChannel + TLS开关)
│
├── delta-test-ai-client  ← AI 服务 HTTP 客户端模块（Java→Python）
│   ├── client/           AiServiceClient（5 个 API 方法：healthCheck/assessRisk/generateSummary/analyzeRootCause/generateCase）
│   ├── config/           AiServiceConfig（@Qualifier("aiRestTemplate") + 5s连接/60s读取超时）
│   └── model/            AiResponse（与 Python R[T] 完全对齐：code/message/data/timestamp）
│
...
└── delta-test-admin      ← 应用启动模块
    ...
    └── resources/
        ...
        ├── logback-spring.xml   ← JSON 结构化日志（dev 彩色控制台 / prod LogstashEncoder）
        ├── openapi.json         ← OpenAPI 3.0 规范快照（前端类型生成源）
        └── db/migration/V1__create_all_tables.sql  ← Flyway 迁移脚本（39 表 + 字典数据）
```
...
│   │   ├── types.generated.ts  ← openapi-typescript 自动生成（paths/components/operations）
│   │   ├── auth.ts          认证 API（loginApi, refreshTokenApi, logoutApi）
│   │   ├── health.ts        健康检查 API（healthCheckApi）
│   │   └── index.ts         统一导出
│   ├── assets/
│   │   └── styles/
...
│   │   ├── llm.py              ← LLM 调用封装（骨架）
│   │   └── logging.py          ← JSON 结构化日志（LOG_FORMAT 环境变量切换）
...
**通信协议**：Java → Python 使用 HTTP REST（JSON），连接超时 5s / 读取超时 60s（AI 耗时较长），客户端类 `AiServiceClient`（`@Qualifier("aiRestTemplate")` 注入）
...
**通信协议**：Java → C 引擎使用 gRPC（Protobuf），连接超时 3s / 读取超时 30s，客户端类 `EngineGrpcClient`

## 1.3 跨语言接口协议汇总

| 方向 | 协议 | 客户端类 | 超时配置 | 说明 |
|------|------|----------|----------|------|
| Vue → Java | HTTP REST + WebSocket | Axios(`http.ts`) + `TaskProgressWebSocketHandler` | 30s / 长连接 | 前端业务请求 + 实时推送 |
| Java → Python | HTTP REST (JSON) | `AiServiceClient` + `@Qualifier("aiRestTemplate")` | 连接 5s / 读取 60s | AI 服务调用，长超时适配 LLM 响应 |
| Java → C | gRPC (Protobuf) | `EngineGrpcClient` + `ManagedChannel` | 连接 3s / 读取 30s | 计算引擎调用 |
| Java → Playwright | RabbitMQ AMQP | `TaskMessageProducer` | 异步无超时 | 任务分发/结果回传 |

**统一响应体对齐**：

| 侧 | 响应体 | 字段 |
|---|---|---|
| Java | `R<T>` | code / message / data / timestamp |
| Python | `R[T]` | code / message / data / timestamp（与 Java `R<T>` 完全一致） |
| Java 反序列化 | `AiResponse` | code / message / data / timestamp（桥接 Python `R[T]` → Java） |

**健康检查端点**：

| 侧 | 端点 | 响应 | 认证 |
|---|---|---|---|
| Java | `GET /api/health` | `R<HealthVO>` (status/service/version) | 无需认证 |
...
│       → AiServiceClient(HTTP) → Python AI(风险评估) → MySQL → 自动匹配用例 → 生成任务  │
...
│  Java → AiServiceClient(HTTP) → Python AI MasterAgent                │
